...
int BuscaFich(EXT_ENTRADA_DIR *directorio, EXT_BLQ_INODOS *inodos, char *nombre){//Lleva a cabo la funcion de buscar el fichero 
...
int Renombrar(EXT_ENTRADA_DIR *directorio, EXT_BLQ_INODOS *inodos,char *nombreantiguo, char *nombrenuevo){//Con esta funcion nos encargamos del comando de renombrar 
    int noesta = 1;
    for(int i = 0; i < MAX_FICHEROS; i++){
        if(strcmp(directorio[i].dir_nfich,nombrenuevo) == 0){//Con este for y el if que le continua nos aseguramos que el nuevo nombre no existe en ningun fichero
            noesta=0;
        }
    }
...
//Se encarga de borrar el fichero
    for(int i = 0; i < MAX_INODOS; i++){
        // encontrar fichero
        if(strcmp(directorio[i].dir_nfich,nombre) ==0){
            // encontrar bloques
...
int Imprimir(EXT_ENTRADA_DIR *directorio, EXT_BLQ_INODOS *inodos,EXT_DATOS *memdatos, char *nombre){//Lleva a cabo el printeo de los datos de un fichero
...
void GrabarDatos(EXT_DATOS *memdatos, FILE *fich){//Graba todos los datos de los ficheros del programa en el fichero
    // abrimos el fichero
    EXT_DATOS datosfich[MAX_BLOQUES_PARTICION];
    rewind(fich);
    fread(&datosfich, SIZE_BLOQUE, MAX_BLOQUES_PARTICION, fich);
